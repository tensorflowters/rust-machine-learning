...
pub fn area_chart(data_to_display: Vec<i32>, title: &str, path_to_save: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.draw_series(
        AreaSeries::new(
            (0..)
                .map(|x| x as f64)
                .zip(data_to_display[..10].iter().map(|x| *x)),
            0,
            &BLUE_200.mix(0.2),
        )
        .border_style(&BLUE_200),
    )?;
